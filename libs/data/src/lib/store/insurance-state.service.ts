...
import { CampoConfiguracionDto, CombinacionComercialDto, ProductoTecnicoConfiguracionDto } from '../dtos/configuracion.dto';

export type DatosPersonaRol = 'tomador' | 'propietario' | 'conductorVehiculo' | 'segundoConductor';

export interface AutoInsuranceData extends BirthDateParts {
	identificador?: number;
	tarea?: string;
	cotizacion?: CotizacionResponseDto;
	productoTecnicoConfigurado?: ProductoTecnicoConfiguracionDto;
	combinacionesComerciales?: CombinacionComercialDto[];
	camposConfiguracion?: CampoConfiguracionDto[];
	...
	letra?: string;
	codigoPostal?: string;
	/** Código postal de localización del riesgo, independiente del domicilio del tomador. */
	codigoPostalVehiculo?: string;
	email?: string;
	paisTelefono?: string;
	...
	esTomadorConductorPrincipal?: boolean;
	esTomadorPropietarioVehiculo?: boolean;
	datosTomador?: DatosPersona;
	datosPropietario?: DatosPersona;
	datosConductorVehiculo?: DatosPersona;
	datosSegundoConductor?: DatosPersona;
...
export class InsuranceStateService {
	...
	activeStepsMap = computed<WizardStep[]>(() => {
		...
		if (data.tipoFlujo === 'MANUAL') steps.push('matricula');
		steps.push('segundo-conductor');
		steps.push('propietario-conductor');
		steps.push('tomador');
		return steps;
	});
	...
	saveDatosTomador(datos: DatosPersona): void {
		this.saveDatosPersonaRol('tomador', datos);
	}

	saveDatosPersonaRol(rol: DatosPersonaRol, datos: DatosPersona): void {
		const datosGuardados = { ...datos };
		switch (rol) {
			case 'tomador':
				this.saveData({ datosTomador: datosGuardados });
				break;
			case 'propietario':
				this.saveData({ datosPropietario: datosGuardados });
				break;
			case 'conductorVehiculo':
				this.saveData({ datosConductorVehiculo: datosGuardados });
				break;
			case 'segundoConductor':
				this.saveData({ datosSegundoConductor: datosGuardados });
				break;
		}
	}
	...
	canContinueFromStep(step: WizardStep): boolean {
		...
		switch (step) {
			...
			case 'propietario-conductor':
				return this.isRelacionTomadorConductorPropietarioValida(data);
			case 'tomador':
				return this.isDatosTomadorValido(data);
			case 'matricula':
				return this.isMatriculaYFechaMatriculacionValidas(data);
	...
	isDatosPersonaRolValido(rol: DatosPersonaRol, data = this._formData()): boolean {
		return validarDatosPersona(this.getDatosPersonaRol(rol, data), TODOS_LOS_CAMPOS_DATOS_PERSONA).valido;
	}

	isRelacionTomadorConductorPropietarioValida(data = this._formData()): boolean {
		return data.esTomadorConductorPrincipal !== undefined && data.esTomadorPropietarioVehiculo !== undefined;
	}

	isMatriculaYFechaMatriculacionValidas(data = this._formData()): boolean {
		return Boolean(data.matricula?.trim()) && Boolean(data.mesPrimerMatricula && data.anioPrimerMatricula);
	}
	...
	getDatosTomador(data = this._formData()): DatosPersona {
		...
		return this.getDatosPersonaRol('tomador', data);
	}

	hasDatosPersonaRol(rol: DatosPersonaRol, data = this._formData()): boolean {
		return getDatosPersonaRolGuardados(rol, data) !== undefined;
	}

	getDatosPersonaRol(rol: DatosPersonaRol, data = this._formData()): DatosPersona {
		const datosGuardados = getDatosPersonaRolGuardados(rol, data);
		return {
			...DATOS_PERSONA_VACIOS,
			...datosGuardados,
			sexo: datosGuardados?.sexo ?? null
		};
	}
...
function getDatosPersonaRolGuardados(rol: DatosPersonaRol, data: AutoInsuranceData): DatosPersona | undefined {
	switch (rol) {
		case 'tomador':
			return data.datosTomador;
		case 'propietario':
			return data.datosPropietario;
		case 'conductorVehiculo':
			return data.datosConductorVehiculo;
		case 'segundoConductor':
			return data.datosSegundoConductor;
	}
}
