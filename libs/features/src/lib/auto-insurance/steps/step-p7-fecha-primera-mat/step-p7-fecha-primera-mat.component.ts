...
import { ElementoGrid, GRID_TARJETAS_DEFAULT_MAX_ELEMENTOS, GridTarjetasComponent } from '@mnv-autos-clientes/ui';
...
export class StepP7FechaPrimeraMatComponent {
	...
	protected readonly yearsSelect = computed(() => this.years().slice(GRID_TARJETAS_DEFAULT_MAX_ELEMENTOS));
	...
	protected onSelectUpdate(event: BalEvents.BalSelectCustomEvent<string | string[] | undefined>): void {
		const yearId = event.detail;
		if (!yearId || Array.isArray(yearId)) return;

		const year = this.yearsSelect().find((item) => item.id === yearId);
		if (year) this.selectYear(year);
	}

	private selectYear(year: ElementoGrid): void {
